...
camera_indices = [0]  # Añade o quita índices según el número de cámaras
    ...
    for capture in video_captures:
        ...
        for face_encoding, face_location in zip(face_encodings, face_locations):
            ...
            if '_' in name:
                ...
                name_without_group = '_'.join(name.split('_')[:-1])
            else:
                group = "No_Grupo"
                name_without_group = name

            # Registrar asistencia
            if group not in attendance_data:
                attendance_data[group] = pd.DataFrame(columns=["Nombre", "Fecha", "Hora"])
            if name != "Desconocido" and name_without_group not in attendance_data[group]["Nombre"].values:
                now = datetime.now()
                new_entry = pd.DataFrame([{"Nombre": name_without_group, "Fecha": now.date(), "Hora": now.time()}])
